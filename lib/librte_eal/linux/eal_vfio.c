...
spapr_dma_win_size(void)
{
	...
	if (rte_eal_iova_mode() == RTE_IOVA_PA) {
		...
		const char str_vmtotal[] = "VmallocTotal:";
		int vmtotal_len = sizeof(str_vmtotal) - 1;
		char buffer[256];
		uint64_t size = 0;
		...
		while (fgets(buffer, sizeof(buffer), fd)) {
			if (strncmp(buffer, str_vmtotal, vmtotal_len) == 0) {
				size = rte_str_to_size(&buffer[vmtotal_len]);
				break;
			}
		}
		fclose(fd);
		...
		if (size == 0) {
			RTE_LOG(ERR, EAL, "Failed to find valid %s entry "
				"in file %s\n", str_vmtotal, proc_meminfo);
			return -1;
		}

		RTE_LOG(DEBUG, EAL, "VmallocTotal is 0x%" PRIx64 "\n", size);
